class RelatedPeopleController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @related_person.save
        ...
        format.json { render action: 'show', status: :created, location: @related_person }
      else
  ...
  def destroy
    ...
    respond_to do |format|
      format.html { redirect_to related_people_url }
      format.json { head :no_content }
    end
  end
  ...
  private
    ...
    def related_person_params
      params.require(:related_person).permit(:title, :description)
    end
end
